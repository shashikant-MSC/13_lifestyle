...
GPT_IMAGE_15_DEFAULT_PARAMS: Dict[str, Any] = {
    "size": "1024x1024",
    "quality": "high",
    "output_format": "webp",
...
OPENAI_IMAGE_MODEL_PARAMS: Dict[str, Dict[str, Any]] = {
    "gpt-image-1.5": GPT_IMAGE_15_DEFAULT_PARAMS,
    "gpt-image-1": {"size": "1024x1024"},
    "gpt-image-1-mini": {"size": "1024x1024"},
}

MODELS = [
    # Trimmed to requested pipeline only:
    "gpt-image-1.5",
    ...
    # Other models disabled intentionally to avoid calls:
    # "gpt-image-1",
    # "gpt-image-1-mini",
...
MODEL_PROMPT_OVERRIDES: Dict[str, str] = {
    "black-forest-labs/flux-2-pro": """You are an expert Lifestyle Content Strategist, Visual Storyteller, and Lifestyle Visual Director specializing in authentic, aspirational, and emotionally transformative content. Your mission is to create a single, high-quality, realistic lifestyle image that visually communicates emotional transformation, aspirational living, and lifestyle storytelling based on the provided input image, its description, and category.
...
3-The image should be 100 percent right as in real world(earphone bud should be properly fixed in ears,no extra wire)
4-Product should not change(any minute deatils should not change like logo,design,color,size(watch dial size should not be enlarged))
5-The person image should not be cropped(Full person should be visible)
6-The image should not look artificial or edited(should look natural and real)
7-the image should follow all physics rules like shadow,light reflection,no floating etc.
8-dont increase the no of products in image(eg-If there is one earphone in input image then output image should also have one earphone not two or more
9-Follow correct photographic rules,product should be on correct angle and position as in real world
10-No extra wires in product (eg-If there is wireless earphone in input image then output image should not have wires)
...
# MODEL_PROMPT_OVERRIDES["gpt-image-1.5"] = "Custom prompt for gpt-image-1.5"
...
# MODEL_PROMPT_OVERRIDES["seedream-4.5"] = "Custom prompt for seedream-4.5"
...
async def seedream_generate(
    ...
    model_id = SEEDREAM_MODEL_IDS.get(model_name, SEEDREAM_MODEL_IDS["seedream-4"])
...
async def generate_with_openai_image_model(
    ...
    model_params = OPENAI_IMAGE_MODEL_PARAMS.get(model_name, {})

    def _call():
        request_body = {"model": model_name, "prompt": prompt_with_reference}
        request_body.update(model_params)
        response = OPENAI_CLIENT.images.generate(**request_body)
        first_image = response.data[0] if getattr(response, "data", None) else None
        if not first_image:
            return None
        # The pipeline expects a single image; if n > 1 is configured, use the first.
...
async def generate_with_replicate_model(
    ...
    def _build_input() -> Optional[Dict[str, Any]]:
        ...
        if model_name == "black-forest-labs/flux-2-pro":
            img = image_arg or reference_url
            if not img:
                print(f"[warn] {model_name} requires an image; none provided.")
                return None
...
MODEL_GENERATORS: Dict[str, Any] = {
    "gpt-image-1.5": generate_with_openai_image_model,
    ...
    "black-forest-labs/flux-2-pro": generate_with_replicate_model,
    ...
    "seedream-4.5": generate_with_seedream_model,
...
async def generate_for_model(
    ...
    template = PROMPT_TEMPLATE_SEEDREAM if model_name in SEEDREAM_MODEL_IDS else PROMPT_TEMPLATE
...
def main() -> None:
    ...
    with st.form("generator_form"):
        ...
        selected_models = st.multiselect(
            "Choose models to generate",
            options=MODELS,
            default=[MODELS[0]],
            help="Only the selected models will be called to avoid unnecessary costs.",
            key="selected_models",
        )
    ...
    if submitted:
        ...
        if not selected_models:
            update_status_text(" Please select at least one model to generate.")
            st.warning("Please select at least one model to generate.")
        elif not collected:
            ...
            try:
                ...
                status_msg = (
                    f" Combined {len(collected)} reference image(s) & generated {produced_outputs} output(s) for {len(selected_models)} model(s)."
                    if produced_outputs
    ...
    if generation:
        ...
        selected_models = generation.get("selected_models") or MODELS
        for model_name, img in zip(selected_models, generation["outputs"]):
